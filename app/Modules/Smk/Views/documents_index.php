    ...
    <div class="row">
            ...
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-body p-4">
                    ...
                    <div class="list-group list-group-flush">
                        <a href="<?= site_url('smk/dokumen') ?>" class="list-group-item list-group-item-action border-0 px-0 <?= !request()->getGet('kategori') ? 'text-purple fw-bold' : '' ?>">
                            <i class="bi bi-grid me-2"></i> Semua Dokumen
                        </a>
                        <?php foreach($nav_doc_categories as $cat): ?>
                            <a href="<?= site_url('smk/dokumen?kategori='.$cat['slug']) ?>" class="list-group-item list-group-item-action border-0 px-0 <?= request()->getGet('kategori') == $cat['slug'] ? 'text-purple fw-bold' : '' ?>">
        ...
        <div class="col-lg-9">
            ...
            <div id="documentResult">
                <?= $this->include('Modules\Smk\Views\documents_data') ?>
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchInput.addEventListener('input', function() {
        ...
        timeout = setTimeout(function() {
            ...
            let url = '<?= site_url('smk/dokumen') ?>' + '?cari=' + encodeURIComponent(keyword);
            if(category) {
