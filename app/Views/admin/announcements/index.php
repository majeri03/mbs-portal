    ...
    <div class="d-flex">
        ...
        <div>
            <?php if (session('school_id')) : ?>
                <strong>Cara Kerja (Floating Widget):</strong>
                <p class="mb-0 mt-1">
                    Pengumuman akan muncul sebagai <strong>Ikon Lonceng (Bubble)</strong> di pojok kanan bawah website sekolah Anda. 
                    Pengunjung dapat mengkliknya untuk melihat detail (Swap Up).
                </p>
            <?php else : ?>
                <strong>Cara Kerja (Auto-Rotate):</strong>
                <p class="mb-0 mt-1">
                    Di Portal Pusat (MBS), pengumuman ditampilkan secara <strong>Bergantian (Running Text)</strong> di bagian atas halaman.
                    Sedangkan jika Anda memposting untuk sekolah, akan mengikuti tampilan sekolah masing-masing.
                </p>
            <?php endif; ?>
        </div>
    </div>
                ...
                <tbody>
                        <?php $no = 1; foreach ($announcements as $announcement) : ?>
                            ...
                            <tr class="<?= $isExpired ? 'table-secondary opacity-50' : '' ?>">
                                <td><?= $no++ ?></td> <td>
                                    <?php if (empty($announcement['school_id'])): ?>
                                        <span class="badge bg-dark">Umum / Pusat</span>
                                    <?php else: ?>
                                        <span class="badge bg-primary"><?= esc($announcement['school_name'] ?? 'Sekolah') ?></span>
                                    <?php endif; ?>
                                </td>

                                <td>
                                    <?php if ($isExpired) : ?>
                                        <span class="badge bg-secondary" title="Sudah melewati tanggal selesai">Expired</span>
                                    <?php elseif ($isActive) : ?>
                                        <span class="badge bg-success" title="Sedang Tampil">Aktif</span>
                                    <?php else : ?>
                                        <span class="badge bg-danger" title="Disembunyikan">Nonaktif</span>
                                    <?php endif; ?>
                                </td>

                                <td>
                                    <div class="fw-semibold text-truncate" style="max-width: 250px;" title="<?= esc($announcement['title']) ?>">
                                ...
                                <td><span class="badge <?= $categoryBadge ?>"><?= $categoryLabel ?></span></td> <td><small><?= date('d M Y', strtotime($announcement['start_date'])) ?></small></td> <td><small><?= date('d M Y', strtotime($announcement['end_date'])) ?></small></td> <td class="text-center"><span class="badge bg-light text-dark border">#<?= $announcement['priority'] ?></span></td> <td class="text-center hide-mobile"><i class="bi <?= esc($announcement['icon']) ?> fs-5 text-secondary"></i></td> <td class="text-center">
    ...
    $(document).ready(function() {
        ...
        $('#announcementsTable').DataTable({
            ...
            "language": {
                ...
                "emptyTable": "<div class='py-4'><i class='bi bi-inbox fs-1 d-block mb-3 opacity-50'></i>Belum ada pengumuman. Klik tombol 'Tambah Pengumuman Baru' di atas!</div>"
            },
            "pageLength": 10,
            "order": [
                [6, "asc"],
                [4, "desc"]
            ], // Sort by priority, then start_date
            "columnDefs": [{
                    "orderable": false,
                    "targets": [0, 8, 9]
                }, // No, Aksi
